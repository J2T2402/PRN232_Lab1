...
public class EnrollmentsController(IEnrollmentService enrollmentService) : ControllerBase
{
    ...
    {
        if (!FieldSelectionHelper.TryValidateFields<EnrollmentResponse>(query.Fields, out var invalidFields))
        {
            return BadRequest(ApiResponse<object>.Fail("Invalid fields requested.", invalidFields));
        }

        var result = await enrollmentService.GetListAsync(ToListQuery(query), cancellationToken);
        if (!result.Success)
        {
            return ToErrorResult(result);
        }

        var items = result.Data!.Items.Select(ResponseMapper.ToEnrollmentResponse).ToList();
    ...
    {
        var result = await enrollmentService.GetByIdAsync(id, expand, cancellationToken);
        if (!result.Success)
        {
            return ToErrorResult(result);
        }

        return Ok(ApiResponse<EnrollmentResponse>.Ok(ResponseMapper.ToEnrollmentResponse(result.Data!), result.Message));
    }
    ...
    {
        var result = await enrollmentService.CreateAsync(new EnrollmentBusinessModel
        {
        ...
        var response = ResponseMapper.ToEnrollmentResponse(result.Data!);
        return StatusCode(StatusCodes.Status201Created, ApiResponse<EnrollmentResponse>.Ok(response, result.Message));
    }
    ...
    [ProducesResponseType(typeof(ApiResponse<object>), StatusCodes.Status404NotFound)]
    [ProducesResponseType(typeof(ApiResponse<object>), StatusCodes.Status500InternalServerError)]
    public async Task<IActionResult> UpdateAsync(int id, [FromBody] UpdateEnrollmentRequest request, CancellationToken cancellationToken)
    {
        var result = await enrollmentService.UpdateAsync(id, new EnrollmentBusinessModel
        {
            StudentId = request.StudentId,
            CourseId = request.CourseId,
            EnrollDate = request.EnrollDate,
    ...
    {
        var result = await enrollmentService.DeleteAsync(id, cancellationToken);
        if (!result.Success)
        {
            return ToErrorResult(result);
        }
    ...
    private static PagedResponse<IEnumerable<object>> ToPagedResponse(IReadOnlyList<object> items, PagedResult<EnrollmentBusinessModel> pagedResult, string message)
        => new()
        {
            Success = true,
            Message = message,
