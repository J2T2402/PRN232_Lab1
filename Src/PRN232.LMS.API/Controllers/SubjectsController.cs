...
public class SubjectsController(ISubjectService subjectService) : ControllerBase
{
    ...
    {
        if (!FieldSelectionHelper.TryValidateFields<SubjectResponse>(query.Fields, out var invalidFields))
        {
            return BadRequest(ApiResponse<object>.Fail("Invalid fields requested.", invalidFields));
        }

        var result = await subjectService.GetListAsync(ToListQuery(query), cancellationToken);
        if (!result.Success)
        {
            return ToErrorResult(result);
        }

        var items = result.Data!.Items.Select(ResponseMapper.ToSubjectResponse).ToList();
    ...
    {
        var result = await subjectService.GetByIdAsync(id, expand, cancellationToken);
        if (!result.Success)
        {
            return ToErrorResult(result);
        }

        return Ok(ApiResponse<SubjectResponse>.Ok(ResponseMapper.ToSubjectResponse(result.Data!), result.Message));
    }
    ...
    [ProducesResponseType(typeof(ApiResponse<object>), StatusCodes.Status400BadRequest)]
    [ProducesResponseType(typeof(ApiResponse<object>), StatusCodes.Status500InternalServerError)]
    public async Task<IActionResult> CreateAsync([FromBody] CreateSubjectRequest request, CancellationToken cancellationToken)
    {
        var result = await subjectService.CreateAsync(new SubjectBusinessModel
        {
        ...
        var response = ResponseMapper.ToSubjectResponse(result.Data!);
        return StatusCode(StatusCodes.Status201Created, ApiResponse<SubjectResponse>.Ok(response, result.Message));
    }
    ...
    [ProducesResponseType(typeof(ApiResponse<object>), StatusCodes.Status404NotFound)]
    [ProducesResponseType(typeof(ApiResponse<object>), StatusCodes.Status500InternalServerError)]
    public async Task<IActionResult> UpdateAsync(int id, [FromBody] UpdateSubjectRequest request, CancellationToken cancellationToken)
    {
        var result = await subjectService.UpdateAsync(id, new SubjectBusinessModel
        {
            SubjectCode = request.SubjectCode,
            SubjectName = request.SubjectName,
            Credit = request.Credit
    ...
    private static PagedResponse<IEnumerable<object>> ToPagedResponse(IReadOnlyList<object> items, PagedResult<SubjectBusinessModel> pagedResult, string message)
        => new()
        {
            Success = true,
            Message = message,
